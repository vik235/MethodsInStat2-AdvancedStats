...
#d) 
#Use Bonferronis at FWER 0.05 to test 4 contrasts. 

#1) Ho: C1 =0 , H1: C1 ~=0 

SSC1 = c1Hat^2/sum((c1^2)/r)

F1=SSC1/MSE
Fcr= qf((1- alpha/4),1,(N-t))

(1- pf(F1,1,(N-t)))/4

#Since F1 > Fcr , We have evidence (pvalue= 1.909548e-07) to reject null and thus the test is significant

#2) Ho: C2 =0 , H1: C2 ~=0 

SSC2 = c2Hat^2/sum((c2^2)/r)
...
#Since F2 > Fcr , We have evidence (pvalue= 1.805769e-10) to reject null and thus the test is significant


#3) Ho: C3 =0 , H1: C3 !=0 

SSC3 = c3Hat^2/sum((c3^2)/r)
...
#Since F3 < Fcr , We have evidence (pvalue= 0.2181045) to reject null and thus the test is significant

#4) Ho: C4 =0 , H1: C4 !=0 

SSC4 = c4Hat^2/sum((c4^2)/r)
...
#Since F4 < Fcr , We have evidence (pvalue= 0.1991412) to reject null and thus the test is significant



# Hypothesis Matrix approach for simultaneously testing 4 contrasts. 

H = matrix(c(c1,c2,c3,c4), nrow = 4, ncol = 5, byrow = TRUE)
mu <- matrix(yi.Bar, nrow = 5, ncol = 1, byrow = TRUE)
xTx <- diag(r,t,t)
Hmu=(H%*%mu)
xTxInverse= solve(xTx)

SSH= t(Hmu)%*%solve(H%*%xTxInverse%*%t(H))%*%Hmu


F=SSH/4/MSE
#F=18.58898
FsimCr <- qf((1- alpha),(t-1),(N-t))
#Critical value of F 2.641465

#Since F > F Critical the test is significant that there exists difference in atleast one of the contrasts. 


#2. Hsu's procedure

#When Best is Smallest 
library("mvtnorm")

m=t-1
dCritical= qmvt(p=1- alpha, tail = "lower.tail", df=N-t, corr = matrix(rep(.5,m^2),m)+diag(m)*.5)$quantile
g <-c()

for (i in 1:length(yi.Bar)) {
mi=min(yi.Bar[(yi.Bar != yi.Bar[i])])  
ki=mi + dCritical*sigmaEHat*sqrt(2/r)
if(yi.Bar[i] <= ki)
  {
  g <- c(g,yi.Bar[i])  
  }
